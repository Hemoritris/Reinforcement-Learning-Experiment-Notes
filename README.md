...
- **REINFORCE 算法**：在 Gym/Gymnasium `CartPole-v1` 环境中实现基于完整轨迹回报的策略梯度学习。
- **随机策略网络**：使用 PyTorch 多层感知机直接参数化动作分布，从状态输出离散动作 logits。
- **折扣回报与标准化**：基于 episode 轨迹计算折扣回报，并通过标准化降低梯度估计方差。
- **训练曲线与模型评估**：自动保存训练奖励曲线、策略网络权重，并使用贪心动作选择评估最终策略表现。
